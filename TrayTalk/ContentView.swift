...
private struct CredentialsSetupSheet: View {
    ...
    @StateObject private var cliSetup = GoogleCloudCLISetupModel()
    ...
    var body: some View {
        ...
        .onChange(of: cliSetup.generatedCredentialsJSON) { _, credentialsJSON in
            guard let credentialsJSON else { return }
            draftCredentials = credentialsJSON
            importedFileName = "Generated by Google Cloud CLI"
            validationState = .idle
            cliSetup.resetGeneratedCredentials()
            validateAndConnect()
        }
    }
    ...
    private var cliSetupContent: some View {
        VStack(alignment: .leading, spacing: 14) {
            GoogleCloudCLISetupPanel(model: cliSetup)
            validationStatus
        }
    }
...
private struct GoogleCloudCLISetupPanel: View {
    @ObservedObject var model: GoogleCloudCLISetupModel
    @State private var selectedBillingAccountID = ""

    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            VStack(alignment: .leading, spacing: 6) {
                Text("Google Cloud CLI")
                    .font(.title2.bold())

                Text("Let Smooth Talker create a new Google Cloud project, enable Text-to-Speech, and generate the service account JSON key.")
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    private var statusPanel: some View {
        HStack(alignment: .top, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                if let gcloudPath = model.gcloudPath {
                    Text(gcloudPath)
    ...
    private var actionPanel: some View {
        ...
        case .idle:
            VStack(alignment: .leading, spacing: 10) {
                Text("Smooth Talker will use an isolated Google Cloud CLI configuration so it does not change your terminal's active gcloud project.")
                    .font(.system(size: 13, weight: .medium))
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)

                Button("Check Google Cloud CLI") {
                    model.checkForCLI()
                }
                .controlSize(.large)
                .buttonStyle(.borderedProminent)
        ...
        case .needsInstall:
            VStack(alignment: .leading, spacing: 10) {
                Text("This downloads Google's official macOS Google Cloud CLI archive into Smooth Talker's Application Support folder.")
                    .font(.system(size: 13, weight: .medium))
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)

                Button("Download and Install Google Cloud CLI") {
                    model.installCLI()
                }
                .controlSize(.large)
                .buttonStyle(.borderedProminent)
        ...
        case .ready:
            Button("Sign In and Create Project") {
                model.startProvisioning()
            }
            .controlSize(.large)
            .buttonStyle(.borderedProminent)
        ...
        case .failed:
            HStack(spacing: 10) {
                ...
                Button(model.failedBecausePythonNeedsUpdate ? "Open Python Download" : "Open Google Billing") {
                    if model.failedBecausePythonNeedsUpdate {
                        model.openPythonDownload()
                    } else {
                        model.openBillingConsole()
                    }
                }
    ...
    private var logsPanel: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            ScrollView {
                Text(model.logs.suffix(24).joined(separator: "\n"))
                    .font(.system(size: 11, weight: .medium, design: .monospaced))
                    .foregroundStyle(.secondary)
                    .frame(maxWidth: .infinity, alignment: .leading)
...
private struct ProgressIcon: View {
    ...
    private var color: Color {
        ...
        case .ready, .readyToValidate:
            return .green
        case .waitingForBilling, .choosingBilling:
            return .orange
        default:
            return .secondary
        }
